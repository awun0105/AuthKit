...
from authkit.authentication.jwt import (
...
def config() -> AuthKitConfig:
  return AuthKitConfig(secret_key="my-super-secret-key-12345678901234567890")

@pytest.fixture
def strict_config() -> AuthKitConfig:
...
class TestPasswordHandler:
  ...
  def test_bcrypt_handler(self, config):
    ...
    bcrypt_config = AuthKitConfig(secret_key="test-secret-at-least-32-bytes-long", password_hasher="bcrypt")
...
class TestJWTHandler:
  ...
  def test_decode_wrong_secret_raises_invalid_token(self, jwt_handler, config):
      token = jwt_handler.create_access_token("user-1")
      other_handler = JWTHandler(AuthKitConfig(secret_key="wrong-secret-key-1234567890123456"))
      with pytest.raises(InvalidToken):
