...
def make_auth(**kwargs) -> AuthKit:
    kwargs.setdefault("require_email_verification", False)
    config = AuthKitConfig(secret_key="phase7-secret-at-least-32-bytes-long", **kwargs)
    return AuthKit(config=config, user_store=MemoryUserStore())
...
def client_for(auth: AuthKit) -> AsyncClient:
    app = make_app(auth)
...
class TestRegisterEndpoint:
    ...
    async def test_register_link_mode_unverified(self):
        auth = make_auth(require_email_verification=True)
        async with client_for(auth) as c:
    ...
    async def test_register_otp_mode_sends_otp(self):
        auth = make_auth(require_email_verification=True, verification_method="otp")
        async with client_for(auth) as c:
            r = await c.post("/auth/register", json={"email": "h@x.com", "password": "strongpass123"})
            assert r.status_code == 201
            user = await auth.store.get_by_email("h@x.com")
...
class TestVerificationEndpoints:
    ...
    async def test_verify_email_link_success(self):
        ...
        async with client_for(auth) as c:
            ...
            user = await auth.store.get_by_email("v1@x.com")
            from itsdangerous import URLSafeTimedSerializer
            s = URLSafeTimedSerializer(auth.config.secret_key, salt="email-verification")
    ...
    async def test_verify_email_already_verified_409(self):
        ...
        async with client_for(auth) as c:
            ...
            user = await auth.store.get_by_email("v2@x.com")
            from itsdangerous import URLSafeTimedSerializer
            s = URLSafeTimedSerializer(auth.config.secret_key, salt="email-verification")
            token = s.dumps(user.email)
    ...
    async def test_verify_otp_success(self):
        ...
        async with client_for(auth) as c:
            ...
            user = await auth.store.get_by_email("v3@x.com")
            otp = generate_otp(6)
            user.verification_otp_hash = hash_token(otp)
            await auth.store.update(user)
    ...
    async def test_verify_otp_attempt_limit_exhausted(self):
        auth = make_auth(require_email_verification=True, verification_method="otp", max_otp_attempts=2)
        async with client_for(auth) as c:
            ...
            user = await auth.store.get_by_email("v5@x.com")
...
class TestLoginEndpoint:
    ...
    async def test_login_by_username(self):
        auth = make_auth(login_identifier_fields=["username", "email"])
        async with client_for(auth) as c:
    ...
    async def test_login_by_phone(self):
        auth = make_auth(login_identifier_fields=["phone", "email"])
        async with client_for(auth) as c:
    ...
    async def test_login_account_locked_423(self):
        auth = make_auth(max_failed_attempts=3)
        async with client_for(auth) as c:
    ...
    async def test_login_mfa_required_403(self):
        auth = make_auth(enable_mfa=True)
        async with client_for(auth) as c:
    ...
    async def test_login_mfa_correct_code_succeeds(self):
        ...
        async with client_for(auth) as c:
            ...
            secret = setup.json()["secret"]
            code = pyotp.TOTP(secret).now()
            confirm = await c.post("/auth/mfa/confirm", json={"totp_code": code}, headers={"Authorization": f"Bearer {token}"})
...
class TestLogoutRefreshProtected:
    ...
    async def test_protected_route_deactivated_user_403(self):
        ...
        async with client_for(auth) as c:
            ...
            token = login.json()["access_token"]
            user = await auth.store.get_by_email("m6@x.com")
...
class TestPasswordEndpoints:
    ...
    async def test_reset_password_link_success(self):
        ...
        async with client_for(auth) as c:
            ...
            user = await auth.store.get_by_email("p3@x.com")
            from itsdangerous import URLSafeTimedSerializer
            s = URLSafeTimedSerializer(auth.config.secret_key, salt="password-reset")
            token = s.dumps(user.email)
    ...
    async def test_reset_password_otp_success(self):
        auth = make_auth(password_reset_method="otp")
        async with client_for(auth) as c:
            await c.post("/auth/register", json={"email": "p4@x.com", "password": "oldpassword"})
            user = await auth.store.get_by_email("p4@x.com")
            otp = generate_otp(6)
    ...
    async def test_set_password_oauth_only_account(self):
        auth = make_auth()
        async with client_for(auth) as c:
            from authkit.models.user import UserInDB as UDB
            user = UDB(email="p9@x.com", hashed_password=None, is_active=True, is_verified=True)
            await auth.store.create(user)
            pair = auth.jwt_handler.create_token_pair(user.id)
...
def make_oauth_auth(**kwargs) -> AuthKit:
    config = AuthKitConfig(
        secret_key="phase7-secret-at-least-32-bytes-long", require_email_verification=False,
        oauth_providers={
...
class TestOAuthEndpoints:
    ...
    async def test_disconnect_last_method_400(self):
        auth = make_oauth_auth()
        async with client_for(auth) as c:
            from authkit.models.user import OAuthAccount
            from authkit.models.user import UserInDB as UDB
            user = UDB(email="oa3@x.com", hashed_password=None, is_active=True, is_verified=True)
            await auth.store.create(user)
            await auth.store.create_oauth_account(OAuthAccount(user_id=user.id, provider="google", provider_user_id="gid"))
            pair = auth.jwt_handler.create_token_pair(user.id)
    ...
    async def test_disconnect_with_password_succeeds(self):
        ...
        async with client_for(auth) as c:
            ...
            token = login.json()["access_token"]
            user = await auth.store.get_by_email("oa5@x.com")
            await auth.store.create_oauth_account(OAuthAccount(user_id=user.id, provider="google", provider_user_id="gid2"))
            r = await c.delete("/auth/oauth/google/disconnect", headers={"Authorization": f"Bearer {token}"})
            assert r.status_code == 204