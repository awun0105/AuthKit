...
from authkit.authentication.encryption import decrypt_token, encrypt_token
from authkit.authentication.jwt import JWTHandler, MemoryTokenBlacklist
from authkit.authentication.oauth import (
    AppleOAuthProvider,
...
from authkit.authentication.oauth_state import MemoryOAuthStateStore, OAuthStateData
...
def config():
    return AuthKitConfig(
        secret_key="phase5-secret-at-least-32-bytes-long",
        oauth_providers={
...
def config_no_autolink():
    return AuthKitConfig(
        secret_key="phase5-secret-at-least-32-bytes-long", auto_link_by_email=False,
        oauth_providers={
            "facebook": OAuthProviderConfig(client_id="fb-cid", client_secret="fb-secret", redirect_uri="https://app.com/cb/facebook"),
        },
    )
...
class TestOAuthAuthorizeFlow:
    ...
    async def test_persists_state(self, config, providers, state_store):
        await oauth_authorize_flow("google", config=config, providers=providers, state_store=state_store)
    ...
    async def test_disabled_provider_raises(self, providers, state_store):
        cfg = AuthKitConfig(secret_key="test-secret-at-least-32-bytes-long", oauth_providers={
