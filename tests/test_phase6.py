"""Phase 6 tests — AuthKit facade, router assembly, dependency injection.
...
from authkit import AuthKit, AuthKitConfig, MemoryUserStore
from authkit.core.config import OAuthProviderConfig
from authkit.models.user import UserInDB


@pytest.fixture
def auth():
    config = AuthKitConfig(secret_key="phase6-secret-at-least-32-bytes-long", require_email_verification=False)
    store = MemoryUserStore()
    return AuthKit(config=config, user_store=store)


@pytest.fixture
def auth_mfa():
    config = AuthKitConfig(secret_key="phase6-secret-at-least-32-bytes-long", require_email_verification=False, enable_mfa=True)
    store = MemoryUserStore()
    return AuthKit(config=config, user_store=store)
...
def auth_oauth():
    config = AuthKitConfig(
        secret_key="phase6-secret-at-least-32-bytes-long", require_email_verification=False,
...
def make_app(auth: AuthKit) -> FastAPI:
    app = FastAPI()
    app.include_router(auth.router, prefix="/auth")

    @app.get("/protected")
    async def protected(user: UserInDB = Depends(auth.current_user)):
        return {"id": user.id, "email": user.email}

    @app.get("/admin-only")
    async def admin_only(_payload=Depends(auth.require_roles("admin"))):
        return {"ok": True}

    @app.get("/write-scope")
    async def write_scope(_payload=Depends(auth.require_scopes("write"))):
        return {"ok": True}
...
class TestRouterAssembly:
    ...
    def test_router_builds_with_20_routes(self, auth):
        def count(router):
            total = 0
...
class TestRegisterLoginFlow:
    ...
    def test_register_returns_201(self, auth):
        client = TestClient(make_app(auth))
    ...
    def test_duplicate_email_409(self, auth):
        client = TestClient(make_app(auth))
    ...
    def test_weak_password_422(self, auth):
        client = TestClient(make_app(auth))
...
class TestProtectedRoutes:
    ...
    def test_current_user_dependency_works(self, auth):
        client = TestClient(make_app(auth))
    ...
    def test_protected_route_no_token_401(self, auth):
        client = TestClient(make_app(auth))
        r = client.get("/protected")
        assert r.status_code == 401  # HTTPBearer auto_error=True, missing credentials

    def test_protected_route_invalid_token_401(self, auth):
        client = TestClient(make_app(auth))
        r = client.get("/protected", headers={"Authorization": "Bearer garbage"})
        assert r.status_code in (400, 401)

    def test_deactivated_user_loses_access(self, auth):
        client = TestClient(make_app(auth))
        ...
        import asyncio
        user = asyncio.run(auth.store.get_by_email("g@x.com"))
        user.is_active = False
        asyncio.run(auth.store.update(user))
...
class TestRolesAndScopes:
    ...
    def _make_token_with(self, auth, roles=None, scopes=None):
        pair = auth.jwt_handler.create_token_pair("fake-user-id", roles=roles or [], scopes=scopes or [])
        return pair.access_token

    def test_require_roles_admin_passes(self, auth):
        client = TestClient(make_app(auth))
        token = self._make_token_with(auth, roles=["admin"])
        r = client.get("/admin-only", headers={"Authorization": f"Bearer {token}"})
        assert r.status_code == 200

    def test_require_roles_non_admin_rejected(self, auth):
        client = TestClient(make_app(auth))
        token = self._make_token_with(auth, roles=["user"])
        r = client.get("/admin-only", headers={"Authorization": f"Bearer {token}"})
        assert r.status_code == 403

    def test_require_scopes_write_passes(self, auth):
        client = TestClient(make_app(auth))
        token = self._make_token_with(auth, scopes=["write"])
        r = client.get("/write-scope", headers={"Authorization": f"Bearer {token}"})
        assert r.status_code == 200

    def test_require_scopes_missing_rejected(self, auth):
        client = TestClient(make_app(auth))
        token = self._make_token_with(auth, scopes=["read"])
        r = client.get("/write-scope", headers={"Authorization": f"Bearer {token}"})
        assert r.status_code == 403
...
class TestLogoutRefresh:
    ...
    def test_logout_returns_204(self, auth):
        client = TestClient(make_app(auth))
    ...
    def test_logout_blacklists_token(self, auth):
        client = TestClient(make_app(auth))
    ...
    def test_refresh_returns_new_pair(self, auth):
        client = TestClient(make_app(auth))
...
class TestChangePassword:
    ...
    def test_change_password_authenticated(self, auth):
        client = TestClient(make_app(auth))
    ...
    def test_change_password_no_auth_401(self, auth):
        client = TestClient(make_app(auth))
...
class TestMFAEndpoints:
    ...
    def test_mfa_setup_returns_secret_and_codes(self, auth_mfa):
        client = TestClient(make_app(auth_mfa))
    ...
    def test_mfa_confirm_activates(self, auth_mfa):
        client = TestClient(make_app(auth_mfa))
        token = self._register_login(client)
    ...
    def test_login_requires_totp_after_mfa_enabled(self, auth_mfa):
        client = TestClient(make_app(auth_mfa))
    ...
    def test_mfa_setup_no_auth_401(self, auth_mfa):
        client = TestClient(make_app(auth_mfa))
...
class TestOAuthEndpoints:
    ...
    def test_authorize_returns_url(self, auth_oauth):
    ...
    def test_authorize_unknown_provider_404(self, auth_oauth):
        client = TestClient(make_app(auth_oauth))
        r = client.get("/auth/oauth/unknownprovider/authorize")
        assert r.status_code == 404

    def test_authorize_with_auth_header_builds_connect_state(self, auth_oauth):
        client = TestClient(make_app(auth_oauth))
    ...
    def test_accounts_list_empty_for_new_user(self, auth_oauth):
        client = TestClient(make_app(auth_oauth))
