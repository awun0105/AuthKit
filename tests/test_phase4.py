...
from authkit.mfa.totp import confirm_mfa_flow, disable_mfa_flow, setup_mfa_flow
from authkit.models.token import TokenPayload
from authkit.models.user import UserCreate
from authkit.permissions.policies import has_scope, require_scopes
...
@pytest.fixture
def config():
    return AuthKitConfig(secret_key="phase4-secret-at-least-32-bytes-long", require_email_verification=False)

@pytest.fixture
def config_mfa():
    return AuthKitConfig(secret_key="phase4-secret-at-least-32-bytes-long", require_email_verification=False, enable_mfa=True)
...
def config_otp_limit():
    return AuthKitConfig(secret_key="phase4-secret-at-least-32-bytes-long", require_email_verification=True,
...
class TestLoginBruteForce:
    ...
    async def test_no_lockout_when_disabled(self, store, config, pw, jwt, notif):
        ...
        cfg = AuthKitConfig(secret_key="test-secret-at-least-32-bytes-long", require_email_verification=False, max_failed_attempts=0)
