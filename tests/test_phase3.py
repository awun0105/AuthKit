...
from authkit.authentication.jwt import JWTHandler, MemoryTokenBlacklist
...
from authkit.notifications.service import NotificationService
from authkit.sms.console import ConsoleSmsBackend
...
@pytest.fixture
def config(): return AuthKitConfig(secret_key="phase3-secret-at-least-32-bytes-long")
@pytest.fixture
def config_otp(): return AuthKitConfig(secret_key="phase3-secret-at-least-32-bytes-long", verification_method="otp", otp_length=6, otp_ttl=600)
@pytest.fixture
def config_no_verify(): return AuthKitConfig(secret_key="phase3-secret-at-least-32-bytes-long", require_email_verification=False)
@pytest.fixture
def config_username(): return AuthKitConfig(secret_key="phase3-secret-at-least-32-bytes-long", login_identifier_fields=["username","email"], require_email_verification=False)
@pytest.fixture
def config_phone(): return AuthKitConfig(secret_key="phase3-secret-at-least-32-bytes-long", login_identifier_fields=["phone","email"], require_email_verification=False)
@pytest.fixture
def config_reset_otp(): return AuthKitConfig(secret_key="phase3-secret-at-least-32-bytes-long", require_email_verification=False, password_reset_method="otp", otp_ttl=600)
@pytest.fixture
...
class TestNotificationServiceRouting:
    ...
    async def test_otp_to_email(self):
        ...
        svc = NotificationService(
            config=AuthKitConfig(secret_key="test-secret-at-least-32-bytes-long", otp_ttl=600),
    ...
    async def test_otp_to_sms(self):
        stream = io.StringIO()
        svc = NotificationService(
            config=AuthKitConfig(secret_key="test-secret-at-least-32-bytes-long", otp_ttl=600, verification_channels=["sms"]),
    ...
    async def test_both_channels(self):
        ...
        svc = NotificationService(
            config=AuthKitConfig(secret_key="test-secret-at-least-32-bytes-long", otp_ttl=600, verification_channels=["email","sms"]),
