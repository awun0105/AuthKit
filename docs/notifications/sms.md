...
| `AWSSNSSmsBackend` | AWS SNS | `boto3` — install via `pip install "authkit[sns]"` |

Unlike email, there's **no `sms_backend` config selector field** — only credential fields (`twilio_account_sid`, etc.) exist on `AuthKitConfig`. You always build and pass the SMS backend instance directly.

## Twilio

```python
from authkit.sms.twilio import TwilioSmsBackend
...
```python
from authkit.sms.sns import AWSSNSSmsBackend
...
AWS credentials resolve from the environment or instance profile — the standard boto3 credential chain, nothing authkit-specific.

## Writing your own backend

```python
from authkit.sms.base import SmsMessage

class MyProvider:
    async def send(self, message: SmsMessage) -> None:
        await my_sms_client.send(to=message.to, body=message.body)

auth = AuthKit(config=config, user_store=store, sms_backend=MyProvider())
```

## Enabling SMS for verification/reset

The backend alone isn't enough — also tell `AuthKitConfig` to actually use it as a channel:
...
auth = AuthKit(config=config, user_store=store, sms_templates=MySmsTemplates())
```
