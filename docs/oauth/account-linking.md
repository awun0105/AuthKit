...
When a social login callback completes, authkit resolves which local user it belongs to using this exact order:
...
What happens here depends on `AuthKitConfig.auto_link_by_email`:
...
A brand-new user. An account is created with `is_active=True, is_verified=True`
and `hashed_password=None`. The authenticated `set-password` endpoint can later
add a password login method. A welcome notification is sent when an email is
available.
...
```
twitter_<provider_user_id>@oauth.authkit.placeholder
```
