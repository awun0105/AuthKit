"""Abstract email backend and message model for authkit.
...
class AbstractEmailBackend(Protocol):
  ...
  - :class:`~authkit.email.console.ConsoleEmailBackend` — prints to stdout
  - :class:`~authkit.email.smtp.SmtpEmailBackend` — sends via SMTP
  - :class:`~authkit.email.sendgrid.SendGridEmailBackend` — SendGrid API
  - :class:`~authkit.email.mailgun.MailgunEmailBackend` — Mailgun API
  """
