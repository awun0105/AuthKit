"""Redis-backed session backend for authkit.
...
class RedisSessionBackend:
  ...
  _SESSION_PREFIX = "authkit:session:"
  _USER_INDEX_PREFIX = "authkit:user_sessions:"

  def __init__(self, redis_url: str) -> None:
      if not _REDIS_AVAILABLE:
          raise ImportError(
              "Redis session backend requires the 'redis' package. "
              "Install it with: pip install authkit[redis]"
          )
  ...
  async def delete_all_for_user(self, user_id: str) -> None:
      ...
      raw_session_ids = await self._client.smembers(index_key)
      session_ids = {
