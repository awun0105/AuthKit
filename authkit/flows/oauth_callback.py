...
from authkit.authentication.encryption import encrypt_token
from authkit.authentication.jwt import JWTHandler
from authkit.authentication.oauth import AppleOAuthProvider, OAuthProviderBase, _token_expiry
...
from authkit.models.user import OAuthAccount, UserInDB, UserRead
...
async def oauth_callback_flow(
    ...
    *,
    ...
    try:
        if isinstance(provider, AppleOAuthProvider):
    ...
    if existing_account is not None:
        ...
        if user is not None:
            ...
            now = utcnow()
            email = userinfo.email or f"{provider_name}_{userinfo.provider_user_id}@oauth.authkit.placeholder"
            user = UserInDB(
                email=email,
