"""JWT access and refresh token management for authkit.
...
from authkit.exceptions import InvalidToken, TokenExpired, TokenRevoked
from authkit.models.token import TokenPair, TokenPayload
from authkit.utils import generate_jti, to_timestamp, utcnow
...
class JWTHandler:
  ...
  def __init__(
    ...
    Args:
        config:    AuthKitConfig — provides secret_key, algorithm, TTLs.
  ...
  def create_token_pair(
    ...
    """
    resolved_family_id = family_id or generate_jti()
    return TokenPair(
        access_token=self.create_access_token(
            user_id, roles, scopes, session_id, resolved_family_id
        ),
